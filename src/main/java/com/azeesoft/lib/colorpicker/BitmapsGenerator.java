...
class BitmapsGenerator {
     ...
     * @return A rectangular Bitmap representing the hue range (0-360), or null if failed
     */
    static Bitmap getHueBitmap(int width, int height) {
     ...
     * @param height    Height of the SatValPicker
     * @param skipCount Number of pixels to skip when generating Bitmap (increasing this results in faster bitmap generation but reduces bitmap quality)
     * @return A Rectangular Bitmap representing the Gradient of Sat and Val for the given Hue, or null if failed
     */
    static Bitmap getSatValBitmap(float hue, int width, int height, int skipCount) {
//        //System.out.println("Width2: " + width);
//        //System.out.println("Height2: " + height);
        try {
            Bitmap satValBitmap = Bitmap.createBitmap(width, height, Bitmap.Config.RGB_565);
...
//                    //System.out.println("Filling...");
            ...
            satValBitmap.setPixels(colors, 0, width, 0, 0, width, height);
            return satValBitmap;
        } catch (Throwable t) {
            return null;
        }
